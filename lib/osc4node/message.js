...
var OscArgument = require('./argument')
...
Message.prototype._add = function(arg) {
    if (!(arg instanceof OscType)) {
        arg = new OscArgument(arg);
    }
    this._typetag += arg.constructor.typeTag;
    this._args.push({ value: arg.value, typetag: arg.constructor.typeTag });
};
...
Message.prototype.decode = function(data) {
    var msg  = this.clone()
      , addr = new (OscArgument.getType('s'))(data);
    
    msg._address = addr.value;
    
    if (data = addr.next) {
        var type = new (OscArgument.getType('s'))(data)
          , typetag = type.value;
        ...
        for (var i = 1; i < typetag.length; i++) {
            var t = new (OscArgument.getType(typetag[i]))(data);
            msg.add(t);
            data = t.next;
        }
    }
